// Vendored, typed copy of public/editors/algorithm/AlgorithmNodes.js's `LIB` registry.
// Kept content-identical to the source so graphs authored here stay compatible with the
// legacy editor and the existing /api/logic runtime chain. Re-sync manually if the source changes.

export type PortType = 'exec' | 'num' | 'bool' | 'string' | 'entity' | 'any' | 'vec2' | 'array';
...
export interface FieldDef {
    key: string;
    label: string;
    type?: 'text' | 'number' | 'color' | 'dropdown';
    source?: string;
}
...
export function getCategories(): string[] {
    ...
    Object.values(NODE_LIBRARY).forEach(def => set.add(def.cat));

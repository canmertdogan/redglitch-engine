...
window.IRAB = {
    ...
    init() {
        ...
        if (window.irab) {
            ...
            window.irab.onCommand = (cmdData) => {
                ...
                if (window.RedGlitchAIInstance && window.RedGlitchAIInstance.toolRegistry) {
                    window.RedGlitchAIInstance.toolRegistry.execute(action, params);
                } else {
                        ...
                        this._flushInterval = setInterval(() => {
                            if (window.RedGlitchAIInstance && window.RedGlitchAIInstance.toolRegistry) {
                                ...
                                while (this._commandQueue.length > 0) {
                                    ...
                                    window.RedGlitchAIInstance.toolRegistry.execute(next.action, next.params);
                                }
                ...
                if (window.RedGlitchEventBus) window.RedGlitchEventBus.emit('ai:command', cmdData);
            };
            ...
            if (window.RedGlitchEventBus) {
                window.RedGlitchEventBus.on('ai:suggestion', (data) => {
                ...
                window.RedGlitchEventBus.on('ai:thought', (data) => {
    ...
    applyPersonaPreset(preset) {
        ...
        const prompts = {
            ...
            'developer': "You are a Senior Engine Developer at RedGlitch. Focus on code efficiency, logic architecture, and technical precision. Professional tone.",

...
class Physics3DWorld {
    ...
    constructor(options = {}) {
        ...
        if (Array.isArray(g)) {
            ...
            this._gravityVec = { x: 0, y: Number(g), z: 0 };
        }
    ...
    init() {
        this.world = new CANNON.World({
            gravity: new CANNON.Vec3(this._gravityVec.x, this._gravityVec.y, this._gravityVec.z),
        });
        ...
        console.log('[Physics3DWorld] init() — gravity:', this._gravityVec, 'step:', this._fixedStep);
        return this;
    }
    ...
    setGravity(x = 0, y = -20, z = 0) {
        this._gravityVec = { x: Number(x), y: Number(y), z: Number(z) };
        if (this.world) {
            this.world.gravity.set(this._gravityVec.x, this._gravityVec.y, this._gravityVec.z);
        }
    }
    ...
    createBody(config = {}) {
        ...
        const initPos = config.position ?? config.mesh?.position ?? { x: 0, y: 0, z: 0 };
        const px = Number(initPos.x ?? initPos[0] ?? 0);
        const py = Number(initPos.y ?? initPos[1] ?? 0);
        const pz = Number(initPos.z ?? initPos[2] ?? 0);

...
class AudioSpatial3D {
     ...
     */
    update(cameraOrDelta, deltaOrCamera) {
        ...
        let delta  = deltaOrCamera;
        if (typeof cameraOrDelta === 'number') {
            delta  = cameraOrDelta;
            camera = deltaOrCamera;
        }
    ...
    /**
     * Legacy compatibility API used by existing 3D engine loops.
     * Updates listener transform directly from numeric coordinates.
     * @param {number} x
     * @param {number} y
     * @param {number} z
     * @param {{x:number,y:number,z:number}|null} [forward]
     * @param {{x:number,y:number,z:number}|null} [up]
     */
    updateListenerPosition(x, y, z, forward = null, up = null) {
        if (!this.ctx || !this._ready) return;

        const lp  = this._listenerPos;
        const lpr = this._listenerPrev;
        const now = performance.now();
        const dt  = this._lastListenerUpdateTS > 0
            ? (now - this._lastListenerUpdateTS) / 1000
            : 0;
        this._lastListenerUpdateTS = now;

        lp.set(x, y, z);
        if (dt > 0) {
            this._listenerVel.subVectors(lp, lpr).divideScalar(dt);
        } else {
            this._listenerVel.set(0, 0, 0);
        }
        lpr.copy(lp);

        const fwd = forward
            ? _compatFwd.set(forward.x, forward.y, forward.z).normalize()
            : _compatFwd.set(0, 0, -1);
        const upVec = up
            ? _compatUp.set(up.x, up.y, up.z).normalize()
            : _compatUp.set(0, 1, 0);
        ...
        _setAudioParam(lis.forwardY, fwd.y, t);
        _setAudioParam(lis.forwardZ, fwd.z, t);
        _setAudioParam(lis.upX, upVec.x, t);
        _setAudioParam(lis.upY, upVec.y, t);
        _setAudioParam(lis.upZ, upVec.z, t);

        for (const em of this._emitters) {
            em._update(lp, this._listenerVel, dt || (1 / 60), t);
        }
        this._updateReverbZones(lp, t);
    }

    // 2D audio compatibility alias used by older engine code.
    updateListener(x, y) {
        this.updateListenerPosition(x, y, 0, null, null);
    }
...
const _compatFwd = new THREE.Vector3();
const _compatUp  = new THREE.Vector3();

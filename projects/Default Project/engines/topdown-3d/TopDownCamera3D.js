...
export default class TopDownCamera3D {
    ...
    /**
     * snapToTarget() — immediately apply current focus target (no lerp).
     */
    snapToTarget() {
        if (!Number.isFinite(this._focusTarget.x) || !Number.isFinite(this._focusTarget.z)) return;
        this._focus.copy(this._focusTarget);
        this._applyCamera(true);
    }

...
class FPSGame extends Engine3DAdapter {
    ...
    constructor(container, options = {}) {
        ...
        this._levelComplete = false;  // one-shot completion guard
        this._initialEnemyCount = 0;  // baseline enemies at level start
    ...
    async init() {
        ...
        // ── HUD (Phase 32) ─────────────────────────────────────────────────
        this.hud = new HUD_FPS(this.container, this.renderer3d?.camera);
    ...
    async onLevelLoaded(level) {
        ...
        if (this.skybox) {
            if (level.skybox) {
                ...
                // fallback: use fog color as solid sky if no skybox defined
    ...
    _update(dt) {
        ...
        // 6b. Skybox — follows camera (Phase 62)
    ...
    _checkCompletionState() {
        if (this._levelComplete || this._initialEnemyCount <= 0) return;

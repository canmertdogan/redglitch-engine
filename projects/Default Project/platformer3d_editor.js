...
const Pf3dEditor = (() => {
    ...
    let Renderer3D = null;
    import('/engines/shared/Renderer3D.js').then(m => { 
        Renderer3D = m.default; 
        window.PrimitiveFactory = m.PrimitiveFactory;
    }).catch(() => {});

    let PaletteManager = null;
    import('/engines/shared/PaletteManager.js').then(m => { 
        PaletteManager = m.default; 
        _palManager = new PaletteManager();
    }).catch(() => {});
    
    let SkyboxSystem = null;
    import('/engines/shared/SkyboxSystem.js').then(m => { SkyboxSystem = m.default; }).catch(() => {});

    let _palManager = null;
    ...
    // Helper to get hex color for index
    function _getColor(idx) {
        if (!_palManager) return 0x888888;
        return _palManager.getColor(idx).getHex();
    }
    ...
    function init() {
        _initThree(() => {
            ...
            if (proj && lvl) {
                ...
                // Also add a default player spawn at (0, 2, 0)
                _state.entities.push({ id: _genId('ent'), type: 'player-spawn', pos: { x: 0, y: 2, z: 0 }, props: {} });
                _rebuildScene3D();
            }
        });
    }
    ...
    function _initThree(callback) {
        const canvas = document.getElementById('viewport-canvas');
        const wrap   = document.getElementById('viewport-wrap');

        if (typeof THREE === 'undefined' || !Renderer3D || !_palManager || !SkyboxSystem) {
            setTimeout(() => _initThree(callback), 50);
            return;
        }

        // Use modern Renderer3D
        const r3d = new Renderer3D(wrap, {
            canvas: canvas,
            outline: true,
            cel: true,
            tones: 3,
        ...
        // Skybox System
        const skybox = new SkyboxSystem(_scene);
        ...
        _three = { renderer3d: r3d, skybox };

        _startRenderLoop();

        // Wire BlockTools (Phase 53) after THREE scene is ready
        if (typeof BlockTools !== 'undefined') {
            BlockTools.init(_scene, _camera, _palManager.toHexArray().map(h => parseInt(h.replace('#',''), 16)));
        }
    ...
    function _startRenderLoop() {
        ...
        const tick = () => {
            ...
            if (_three?.skybox) _three.skybox.update(_camera);
    ...
    function setBlockColorIdx(idx) {
        ...
        const hex = '#' + _getColor(_blockColorIdx).toString(16).padStart(6, '0');
        document.getElementById('color-swatch-preview').style.background = hex;
        if (_ghostMesh) _ghostMesh.material.color.setHex(_getColor(_blockColorIdx));
    }
    ...
    function _rebuildGhostGeometry() {
        ...
        _ghostMesh.material.color.setHex(_getColor(_blockColorIdx));
    }
    ...
    function _showProperties(id, group) {
        ...
        propsBody.innerHTML = `
            ...
            <div class="prop-row"><label>Hex Color</label>
                <input type="color" value="${_getObjHex(obj)}"
                    onchange="Pf3dEditor.setPropColorHex('${id}', this.value)">
            </div>
            <div class="prop-row"><label>Texture</label>
                <select onchange="Pf3dEditor.setPropTexture('${id}', this.value)">
                    <option value="" ${!obj.textureId ? 'selected' : ''}>None (Solid)</option>
                    ${(() => {
                        if (typeof BlockTools === 'undefined' || !BlockTools.getAvailableTextures) return '';
                        return BlockTools.getAvailableTextures().map(t => 
                            `<option value="${t}" ${obj.textureId === t ? 'selected' : ''}>${t.charAt(0).toUpperCase() + t.slice(1)}</option>`
                        ).join('');
                    })()}
                </select>
            </div>
    ...
    function setPropColorHex(id, hex) {
        const obj = _state.objects.find(o => o.id === id);
        if (!obj) return;
        _pushUndo();
        obj.colorHex = hex;
        _updateMeshMaterial(id);
        _markDirty();
    }

    function setPropTexture(id, textureId) {
        const obj = _state.objects.find(o => o.id === id);
        if (!obj) return;
        _pushUndo();
        obj.textureId = textureId || null;
        _updateMeshMaterial(id);
        _markDirty();
    }

    function _updateMeshMaterial(id) {
        const obj  = _state.objects.find(o => o.id === id);
        const mesh = _meshMap.get(id);
        if (!obj || !mesh) return;
        ...
        if (typeof BlockTools !== 'undefined') {
            // Rebuild using tools
            mesh.material = BlockTools.buildMaterialForObj(obj, _state.wireframe);
            // Apply UVs if texture changed
            if (obj.textureId && mesh.geometry.type === 'BoxGeometry') {
                BlockTools.applyTextureUVs(mesh.geometry, obj.textureId);
            }
        } else {
            mesh.material.color.set(obj.colorHex || '#888888');
        }
    }

    function _getObjHex(obj) {
        if (obj.colorHex) return obj.colorHex;
        return '#' + _getColor(obj.colorIdx ?? 122).toString(16).padStart(6, '0');
    }
    ...
    function _addObjectMesh(obj) {
        ...
        if (typeof BlockTools !== 'undefined') {
            ...
            const color = _getColor(obj.colorIdx ?? 12);
    ...
    function _addEntityMesh(en) {
        ...
        const mesh  = new THREE.Mesh(geo, mat);
        // player-spawn is a cone with height 1.2, set y slightly higher (0.6 instead of 0.5)
        const offset = (en.type === 'player-spawn') ? 0.6 : 0.5;
        mesh.position.set(en.pos.x, en.pos.y + offset, en.pos.z);
    ...
    function _loadMapData(data, project, levelId) {
        ...
        _state.objects   = (data.platforms || []).map(p => ({
            ...
            colorIdx:  p.colorIdx ?? 122,
            colorHex:  p.colorHex || null,
            textureId: p.textureId || null,
    ...
    function _buildLevelPayload() {
        ...
        return {
            ...
            platforms:   _state.objects.map(o => ({
                ...
                subtype: o.subtype || 'flat',
                blockType: o.blockType || 'box',
                x: o.pos.x, y: o.pos.y - (o.scale.y || 1) / 2, z: o.pos.z,
                w: o.scale.x, h: o.scale.y, d: o.scale.z,
                colorIdx:  o.colorIdx,
                colorHex:  o.colorHex || null,
                textureId: o.textureId || null,
                rot: o.rot,
                ...(o.props || {}),
            })),
            paths,

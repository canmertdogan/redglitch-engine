...
router.post('/save-level', async (req, res) => {
    try {
        ...
        // Save level
        await fs.writeFile(levelPath, JSON.stringify(data, null, 2));
            ...
            try {
                ...
                for (const file of jsonFiles) {
                    try {
                        ...
                        let engineType = data.engineType || data.metadata?.engineType || data.type;
                        
                        // Intelligent fallback detection ONLY if no type is explicitly set
                        if (!engineType) {
                            if (data.layers && Array.isArray(data.layers)) {
                                engineType = 'rpg-topdown';
                            } else if (data.spawn && typeof data.spawn === 'object' && data.collision && Array.isArray(data.collision)) {
                                // Platformer levels usually have spawn and collision but NO layers
                                engineType = 'platformer-2d';
                            } else if (data.grid && Array.isArray(data.grid)) {
                                engineType = 'iso-pixel';
                            } else {
                                engineType = 'rpg-topdown'; // Final default
                            }
                        }
                        
                        // Map shorthand/legacy types to standardized engine IDs
                        let standardizedType = engineType;
                        if (engineType === 'topdown' || engineType === 'rpg') standardizedType = 'rpg-topdown';
                        else if (engineType === 'iso-pixel' || engineType === 'iso') standardizedType = 'iso-pixel';
                        else if (engineType === 'platformer' || engineType === 'plt') standardizedType = 'platformer-2d';
